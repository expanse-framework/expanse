...
def list_users(
    ...
    :raises HTTPException: (403) The user is not authorized to list users.
    """
    stmt = select(User)
...
def create_user(
    ...
    Raises:
        HTTPException: (403) The user is not authorized to create a user.
        HTTPException: (422) The request data is invalid.
    """
...
def update_user(
    ...
    :raises HTTPException: (403) The user is not authorized to update a user.
    :raises HTTPException: (404) The user is not found.
    :raises HTTPException: (422) The request data is invalid.
    """
...
def delete_user(session: Session, user_id: int) -> Response:
    ...
    :raises HTTPException: (403) The user is not authorized to delete a user.
    :raises HTTPException: (404) The user is not found.

    :returns: 204 No Content response on successful deletion.
    """
    user = session.get(User, user_id)
    if user is None:
        abort(404, "User not found")
