...
from tests.support.exceptions.helpers import nested_exception
from tests.support.exceptions.helpers import simple_exception
...
def test_frame() -> None:
    ...
    assert repr(frame) == f"<Frame {__file__}, test_frame, 10>"
...
def test_frame_with_no_context_should_return_empty_line() -> None:
    frame = Frame(inspect.FrameInfo(None, "filename.py", 123, "function", None, 3))  # type: ignore[arg-type]
