...
from expanse.http.middleware.check_csrf_token import CheckCSRFToken
...
def test_middleware_is_passthrough_for_read_queries(
    ...
    router.get("/", handler).middleware(LoadSession, CheckCSRFToken)
...
async def test_middleware_retrieves_token_from_form_data(
    ...
    router.post("/", handler).middleware(LoadSession, CheckCSRFToken)
