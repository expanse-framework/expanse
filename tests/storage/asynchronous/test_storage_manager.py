...
async def test_storage_manager_delegates_get(
    manager: StorageManager, tmp_path: Path
) -> None:
    (tmp_path / "hello.txt").write_bytes(b"hello")

    result = await manager.get("hello.txt")

    assert result == b"hello"


async def test_storage_manager_delegates_put(
    manager: StorageManager, tmp_path: Path
) -> None:
    await manager.put("greeting.txt", b"world")

    assert (tmp_path / "greeting.txt").read_bytes() == b"world"


async def test_storage_manager_delegates_exists(
    manager: StorageManager, tmp_path: Path
) -> None:
...
async def test_storage_manager_delegates_delete(
    manager: StorageManager, tmp_path: Path
) -> None:
    (tmp_path / "to_delete.txt").write_bytes(b"bye")

    await manager.delete("to_delete.txt")

    assert not (tmp_path / "to_delete.txt").exists()


async def test_storage_manager_delegates_list(
    manager: StorageManager, tmp_path: Path
) -> None:
...
async def test_storage_manager_delegates_size(
    manager: StorageManager, tmp_path: Path
) -> None:
    (tmp_path / "sized.txt").write_bytes(b"12345")

    assert await manager.size("sized.txt") == 5


async def test_storage_manager_delegates_copy(
    manager: StorageManager, tmp_path: Path
) -> None:
    (tmp_path / "source.txt").write_bytes(b"copied")

    await manager.copy("source.txt", "dest.txt")

    assert (tmp_path / "source.txt").read_bytes() == b"copied"
    assert (tmp_path / "dest.txt").read_bytes() == b"copied"


async def test_storage_manager_delegates_move(
    manager: StorageManager, tmp_path: Path
) -> None:
    (tmp_path / "original.txt").write_bytes(b"moved")

    await manager.move("original.txt", "renamed.txt")
