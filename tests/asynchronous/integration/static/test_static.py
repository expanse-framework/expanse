...
async def test_endpoint_returns_static_content_successfully(client: TestClient) -> None:
    provider = StaticServiceProvider(client.app)
...
async def test_view_static_function_is_registered_successfully(
    ...
    await client.app.register(ViewServiceProvider(client.app))
    await client.app.register(provider)
    await provider.boot()

    async def foo(view: ViewFactory) -> Response:
        return await view.make("foo")

    (await client.app.make(Router)).add_route(Route.get("/foo", foo))
