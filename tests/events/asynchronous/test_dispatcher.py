...
from expanse.events.asynchronous.dispatcher import EventDispatcher
...
async def stopping_listener(event: MyEvent) -> Literal[False]:
    logger.info("stopping_listener called")

    return False  # This will stop the propagation of the event


async def test_dispatcher_can_call_async_listener(caplog: LogCaptureFixture) -> None:
...
async def test_dispatcher_can_call_sync_listener(caplog: LogCaptureFixture) -> None:
    caplog.set_level(logging.INFO)
...
async def test_dispatcher_stops_on_false_return(caplog: LogCaptureFixture) -> None:
    caplog.set_level(logging.INFO)
