...
from expanse.routing.trie.finder import Finder
...
class Router:
    ...
    def add_route(self, route: Route) -> None:
        for method in route.methods:
            self._finder.route(method, route.path, route)
    ...
    def find(self, name: str) -> Route | None:
        return self._finder.find_by_name(name)

    async def handle(self, container: Container, request: Request) -> Response:
        route, params = self._finder.find(request.method, request.url.path)
