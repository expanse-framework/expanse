...
class Router(RouterContract):
    ...
    def _route_handler(self, route: Route, container: Container) -> RequestHandler:
        async def handler(request: Request) -> Response:
            ...
            arguments, unbound_arguments = await compiled.bind(request)
            ...
            unbound_arguments = [
                name for name in unbound_arguments if name not in keywords
            ]

            # arguments.update(compiled.bind_query(request, unbound_arguments))
