...
class AsyncDatabaseStore(AsyncStore):
    def __init__(
        self,
    ...
    async def write(
        ...
        match engine.dialect.name:
            case "sqlite":
                await self._sqlite_upsert(session_id, payload)
            case "postgresql":
                await self._postgres_upsert(session_id, payload)
            case "mysql":
                await self._mysql_upsert(session_id, payload)
            case _:
                async with self._db.connection(self._database_name) as connection:
                    session_exists = (
                        await connection.execute(
    ...
    async def _postgres_upsert(self, session_id: str, payload: dict[str, Any]) -> None:
    ...
    async def _sqlite_upsert(self, session_id: str, payload: dict[str, Any]) -> None:
