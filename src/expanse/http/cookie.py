...
class Cookie:
    def __init__(
        ...
        self._name = name
        self._value = value
        self._expires = self._compute_expires(expires)
        self._domain = domain
        self._path = path
        self._secure = secure
    ...
    def max_age(self) -> int:
        max_age: int = int((self._expires - datetime.now(timezone.utc)).total_seconds())
    ...
    def _compute_expires(self, expires: int | datetime) -> datetime:
    ...
    def __str__(self) -> str:
        ...
        if not self._value:
            ...
            if self._expires and self.max_age > 0:
                parts.append(
                    f"; expires={self._expires.strftime('%a, %d %b %Y %H:%M:%S GMT')}; Max-Age={self.max_age}"
                )
